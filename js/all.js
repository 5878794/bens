...
$(document).ready(function () {
    ...
    //解决当中间部分高度不够的时候，页脚到不了底部
    if ($("#footer").length != 0) {
        var oFooterHeight = $("#footer").offset().top;
        var oHeight = $(window).height() - $("#footer").height();
        if (oFooterHeight < oHeight) {
            $("#footer").css("marginTop", $(window).height() - oFooterHeight - $("#footer").height());
        } else {
            return;
        }
    } else {
...
//创建弹出层的包裹层样式
TGO.createWrapDiv = function (width) {
    var div = $("<div></div>");
    div.css({
        padding: "10px",
...
//店铺等滚动加载专用
$.scrollLoadInterval = function (opt, obj_) {
    ...
    obj = new TGO.scrollLoad(param);
    obj.ajaxStart();
    //param.ajaxFn();

    if (obj_) {
        window[obj_] = obj;
        obj.scrollFn.active = false;
    }
};
...
function ShowShare(_sharetype) {
    ...
    if ($(".share-content").length > 0) {
        $.ajax({
            type: "POST",
            data: { sharetype: _sharetype, url: _url, shareid: $(".shareid").text(), sharetitle: $(".sharetitle").text(), sharedescription: $(".sharedescription").text(), shareimage: $(".shareimage").text() },
            dataType: "json",
...
function SharePro(_sharetype, pname) {
    ...
    var shareimage;//图片
    sharetitle = $("#baby_" + pname).find(".sharetitle").text();
    sharedescription = $("#baby_" + pname).find(".sharedescription").text();
    shareimage = $("#baby_" + pname).find(".shareimage").text();
    $.ajax({
        type: "POST",
...
function ShareSpecial(_sharetype, pname) {
    ...
    var sharedescription;//内容
    var shareimage;//图片
    _url = $("#baby_" + pname).find(".shareUrl").text();
    sharetitle = $("#baby_" + pname).find(".sharetitle").text();
    sharedescription = $("#baby_" + pname).find(".sharedescription").text();
    shareimage = $("#baby_" + pname).find(".shareimage").text();
    $.ajax({
...
function share(obj) {
    ...
    if ($(".share-content").length > 0) {
        var title = $(".share-content").find(".sharetitle").text(); //标题
        var sharedescription = $(".share-content").find(".sharedescription").text();//内容
        var pic = $(".share-content").find(".shareimage").text(); //图片
        if (sharedescription != "" && sharedescription.length > 0) {
            title = title + "--" + sharedescription;
        }
        var _URL;
        if (obj == "sina") {
        ...
        //location.href=_URL;
        window.open(_URL, null, 'height=570,width=657,top=' + (screen.height - 500) / 2 + ',left=' + (screen.width - 800) / 2 + ',toolbar=no,menubar=no,scrollbars=yes,resizable=no,location=no,status=no'); //重新打开一个新的窗体
    }
}
...
(function () {
    ...
    upload_file.prototype = {
        ...
        oldSuccess: function (rs) {
            ...
            var src = rs.Data;
            //TODO 因跨域临时写成绝对地址,发布后删除
            //src = "http://172.18.254.158:8023" + src;
...
$.fn.showRemainderTime = function (server_time, start_time, end_time, number) {
    var temp_time = new Date().getTime(),
        diff_time = server_time - temp_time,
        fn = null,
        run = true;
    ...
    var get_time = function () {
        ...
        if (now_time < start_time) {
            var remain_time = start_time - now_time;
            var d = parseInt(remain_time / 86400000);
            remain_time = remain_time - (d * 86400000);
            var h = parseInt(remain_time / 3600000);
            remain_time = remain_time - (h * 3600000);
            var m = parseInt(remain_time / 60000);
            remain_time = remain_time - (m * 60000);
            var s = parseInt(remain_time / 1000);
            temp = {
                d: d,
                h: h,
                m: m,
            ...
            return "剩余: <em>" + temp.d + "</em>天<em>" + temp.h + "</em>小时<em>" + temp.m + "</em>分<em>" + temp.s + "</em>秒";
        }
    }
