...
//
// 	//图片滤镜
// 	async filter(src,type,opt){
// 		if(!src || !type){return;}
// 		if(!filterFns[type]){throw '滤镜不存在'}
//
// 		let data = await this[getImageData](src),
// 			{filterFn, newImage} = filterFns[type](data,opt);
//
...
// 				let rgba = filterFn(x,y,width,height);
// 				this[setRGBA](newImage,x,y,rgba);
// 			}
// 		}
//
